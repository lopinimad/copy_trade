...
class AsyncPocketOptionClient:
    ...
    async def delete_order_result(self, order_id: str) -> Optional[OrderResult]:
        """
        Check the result of a specific order
        ...
        if "requestId" in data and "asset" in data and "amount" in data:
            ...
            if (
                ...
                order_result = OrderResult(
                    ...
                    duration=int(float(data.get("closeTimestamp", 0)) - float(data.get("openTimestamp", 0))),
                    status=OrderStatus.ACTIVE,
                    placed_at=float(data.get("openTimestamp", 0)),
                    expires_at=float(data.get("closeTimestamp", 0)),
